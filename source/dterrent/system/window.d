...
class Window : IRenderTarget
{
	...
	private this(int width, int height)
	{
		...
		sdlWindow = SDL_CreateWindow("DTERRENT", cast(int) SDL_WINDOWPOS_UNDEFINED,
				cast(int) SDL_WINDOWPOS_UNDEFINED, cast(int) winWidth,
				cast(int) winHeight, cast(SDL_WindowFlags) flags); // TODO depth, is no longer an option.
		...
		if (sdlSurface is null)
			throw new Exception(format("Unable to set %d x %d video mode: %s ",
