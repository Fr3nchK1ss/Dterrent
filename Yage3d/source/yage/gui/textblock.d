		...
		foreach(i, l; letters.data)
		{
			...
			if (currentStyle != *newStyle) // if style has changed since last letter
			{
				...
				if (style.fontFamily && style.fontFamily != newStyle.fontFamily)
					styleString ~= format(`font-family: url('%s')`, newStyle.fontFamily);
				if (dword(style.fontSize) != dword(newStyle.fontSize))
					styleString ~= format(`font-size: %spx`, newStyle.fontSize);
				if (style.color != newStyle.color)
					styleString ~= format(`color: %spx`, newStyle.color);
				if (style.fontWeight != newStyle.fontWeight)
					styleString ~= format(`font-weight: %s`, Style.enumToString(newStyle.fontWeight));
				if (style.fontStyle != newStyle.fontStyle)
					styleString ~= format(`font-style: %s`, Style.enumToString(newStyle.fontStyle));
				if (style.textDecoration != newStyle.textDecoration)
					styleString ~= format(`text-decoration: %s`, Style.enumToString(newStyle.textDecoration));
				if (dword(style.lineHeight) != dword(newStyle.lineHeight))
					styleString ~= format(`lineHeight: %spx`, newStyle.lineHeight);
				if (dword(style.letterSpacing) != dword(newStyle.letterSpacing))
					styleString ~= format(`letterSpacing: %spx`, newStyle.letterSpacing);

				html ~= format(`</span><span style="%s">`, join(styleString, "; "));
